...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # pending
        lost1  = self._make_lost(alice,   'Black Laptop Bag',            'bags',            'Left near the library entrance on the bench outside',        today - timedelta(days=3),  'pending')
        lost2  = self._make_lost(alice,   'AirPods Pro White',           'electronics',     'Dropped somewhere in Block A canteen during lunch',          today - timedelta(days=7),  'pending')
        lost3  = self._make_lost(ben,     'Calculus Textbook',           'books_stationery','Left in Lecture Hall 3 after the morning session',           today - timedelta(days=5),  'pending')
        lost4  = self._make_lost(ben,     'Blue Water Bottle',           'other',           'Sports complex, near the football field',                    today - timedelta(days=2),  'pending')
        lost5  = self._make_lost(claire,  'Silver Bracelet',             'accessories',     'Somewhere between Block B and the student canteen',          today - timedelta(days=9),  'pending')
        lost6  = self._make_lost(claire,  'Prescription Glasses',        'other',           'Faculty of Science, Room 101 or the corridor outside',       today - timedelta(days=1),  'pending')
        lost7  = self._make_lost(dr_nair, 'Staff ID Card',               'id_cards',        'Possibly dropped near the Faculty of Engineering office',    today - timedelta(days=4),  'pending')
        lost8  = self._make_lost(mr_paul, 'Car Keys Toyota Red Keyring', 'keys',            'Staff car park or main admin building corridor',             today - timedelta(days=6),  'pending')

        # found (claim submitted)
        lost9  = self._make_lost(alice,   'UoM Student ID Card',         'id_cards',        'Lost around the Faculty of Engineering reception',           today - timedelta(days=10), 'found')
        lost10 = self._make_lost(ben,     'Black Hoodie Size M',         'clothing',        'Student lounge on the ground floor of Block C',             today - timedelta(days=12), 'found')

        # resolved
        lost11 = self._make_lost(claire,  'Wireless Mouse',              'electronics',     'Computer lab 3, left on desk after project session',         today - timedelta(days=20), 'resolved')
        lost12 = self._make_lost(dr_nair, 'Umbrella Navy Blue',          'other',           'Left at the staff common room',                              today - timedelta(days=18), 'resolved')

        # ── Found Items ────────────────────────────────────────────────────
        self.stdout.write('\nSeeding found items...')

        # pending
        found1  = self._make_found(ben,     'Black Leather Wallet',             'accessories',     'Found near the main gate security desk on the pavement',     today - timedelta(days=2),  'pending')
        found2  = self._make_found(dr_nair, 'Set Of Keys Blue Lanyard',         'keys',            'Found on a bench outside Block C lecture rooms',             today - timedelta(days=4),  'pending')
        found3  = self._make_found(claire,  'Scientific Calculator Casio',      'electronics',     'Found in Lab 204 after the afternoon practical',             today - timedelta(days=3),  'pending')
        found4  = self._make_found(mr_paul, 'Red Jacket Size L',                'clothing',        'Left on a chair in the staff meeting room, Block A',         today - timedelta(days=7),  'pending')
        found5  = self._make_found(alice,   'Earphones Wired Black',            'electronics',     'Found on a library desk, floor 2',                           today - timedelta(days=1),  'pending')
        found6  = self._make_found(ben,     'Green Pencil Case',                'books_stationery','Found near the vending machines on ground floor',            today - timedelta(days=5),  'pending')
        found7  = self._make_found(claire,  'Student ID Card Found',            'id_cards',        'Found in the library reading room between books',            today - timedelta(days=1),  'pending')
        found8  = self._make_found(dr_nair, 'Phone Charger USB C',              'electronics',     'Left plugged in at a socket in the postgrad lounge',         today - timedelta(days=8),  'pending')

        # claimed
        found9  = self._make_found(mr_paul, 'Prescription Glasses Black Frame', 'accessories',     'Found on the staircase between floors 1 and 2, Block B',    today - timedelta(days=11), 'claimed')
        found10 = self._make_found(alice,   'Laptop Power Adapter Dell',        'electronics',     'Found near a charging station in the student lounge',        today - timedelta(days=9),  'claimed')

        # resolved
        found11 = self._make_found(ben,     'Black Backpack Adidas',            'bags',            'Found outside the canteen near the recycling bins',          today - timedelta(days=22), 'resolved')
        found12 = self._make_found(claire,  'Student Bus Pass',                 'id_cards',        'Found on a seat in Lecture Theatre 1',                       today - timedelta(days=19), 'resolved')
        found13 = self._make_found(dr_nair, 'Silver Ring',                      'accessories',     'Found on the floor of the ladies bathroom, Block A',         today - timedelta(days=25), 'resolved')

        # ── Claims ─────────────────────────────────────────────────────────
        self.stdout.write('\nSeeding claims...')

        # pending
        self._make_claim(ben,     None,    found9,  'Those are my glasses — black rectangular frame with a small scratch on the left arm.',     'pending')
        self._make_claim(claire,  None,    found10, 'I lost my Dell adapter last week. The brick has a sticker with my student ID S21003.',      'pending')
        self._make_claim(alice,   lost9,   None,    'That is my ID card. My photo and student number S21001 should be on it.',                   'pending')
        self._make_claim(ben,     lost10,  None,    'I lost a black hoodie. Mine has a small bleach stain on the left sleeve.',                  'pending')

        # returned
        self._make_claim(claire,  lost11,  None,    'My wireless mouse, I left it in the lab after the group project.',                          'returned')
        self._make_claim(dr_nair, lost12,  None,    'That is my navy umbrella, I left it in the common room on Tuesday.',                        'returned')
        self._make_claim(alice,   None,    found11, 'Black Adidas backpack is mine — has my initials A.S. written inside the top pocket.',       'returned')
        self._make_claim(claire,  None,    found12, 'That is my bus pass, my name Claire Dubois and photo are on it.',                           'returned')
        self._make_claim(alice,   None,    found13, 'The silver ring belongs to me, it has an engraving inside that reads "forever".',           'returned')

        # rejected
        self._make_claim(ben,     None,    found3,  'I think I left my calculator there but I am not 100% sure of the model.',                   'rejected')
        self._make_claim(mr_paul, None,    found1,  'Could be my wallet, I lost one last week around that area.',                                'rejected')
        ...
        r4 = self._make_review(dr_nair, 4, 'As a staff member I use this regularly. Logging found items is quick. A notification when a claim is resolved would be a great addition.')
    ...
    def _get_image_path(self, item_name, folder):
        """
        Converts an item name to a filename and checks if it exists under MEDIA_ROOT.
        e.g.  'Black Laptop Bag'  ->  media/lost_items/black_laptop_bag.png
        Returns the relative path string (stored in the DB photo field) or None.
        """
        clean = (
            item_name.lower()
            .replace(' ', '_')
